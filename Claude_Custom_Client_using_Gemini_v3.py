...
# ── Type map shared across all conversion helpers ────────────────────────────
_TYPE_MAP: dict = {
    "string":  "STRING",
    "integer": "INTEGER",
    "number":  "NUMBER",
...
def _resolve_json_type(json_type) -> str:
    """
    Normalise a JSON Schema 'type' value to a single lowercase string.

    JSON Schema allows two forms:
      * A string : "string"            -> return as-is
      * A list   : ["string", "null"]  -> pick first non-null entry

    Gemini only accepts a single type per field, so we always reduce to one.
    """
    if isinstance(json_type, list):
        non_null = [t for t in json_type if t != "null"]
        return non_null[0] if non_null else "string"
    return json_type if isinstance(json_type, str) else "string"


def _get_gemini_type(json_type_str: str):
    """Return the genai_types.Type enum value for a JSON Schema type string."""
    type_attr = _TYPE_MAP.get(json_type_str, "STRING")
    return getattr(genai_types.Type, type_attr)


def _convert_schema(prop_schema: dict) -> genai_types.Schema:
    """
    Recursively convert one JSON Schema node into a Gemini Schema object.

    Handles three structural cases Gemini strictly validates:
      1. OBJECT  -> must include 'properties' (recursively converted)
      2. ARRAY   -> must include 'items' (Gemini rejects arrays without it)
      3. Scalar  -> STRING / INTEGER / NUMBER / BOOLEAN

    The PowerBI MCP server produces array schemas with no 'items' field
    (valid JSON Schema but rejected by Gemini). We inject a STRING items
    schema as a safe default in that case.
    """
    raw_type    = prop_schema.get("type", "string")
    json_type   = _resolve_json_type(raw_type)
    gemini_type = _get_gemini_type(json_type)
    description = prop_schema.get("description", "")

    # Case 1: OBJECT
    if gemini_type == genai_types.Type.OBJECT:
        nested_props = {}
    ...
    # Case 2: ARRAY — Gemini REQUIRES an items field on every array schema.
    # The PowerBI MCP server often omits it (valid JSON Schema, invalid for Gemini).
    # We recursively convert the items sub-schema if present, or fall back
    # to a STRING items schema as the safest possible default.
    if gemini_type == genai_types.Type.ARRAY:
        if prop_schema.get("items"):
            items_schema = _convert_schema(prop_schema["items"])
        else:
            # This is the core fix for all the "missing field" errors.
            # Gemini must know what type lives inside the array.
            items_schema = genai_types.Schema(type=genai_types.Type.STRING)
        return genai_types.Schema(
            type=gemini_type,
            description=description,
            items=items_schema,
    ...
    # Case 3: Scalar (STRING / INTEGER / NUMBER / BOOLEAN)
    return genai_types.Schema(
        type=gemini_type,
        description=description,
    )
...
def _convert_properties(props: dict) -> dict[str, genai_types.Schema]:
    """
    Convert a JSON Schema 'properties' dict into a Gemini Schema dict.
    Each value is passed through _convert_schema() for full recursive handling.
    """
    return {
        prop_name: _convert_schema(prop_schema)
